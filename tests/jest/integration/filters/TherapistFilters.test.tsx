import React from "react";
import { render, screen, fireEvent } from "@testing-library/react";
import TherapistFilters from "@/components/filters/TherapistFilters";

// Mock next/navigation
const mockSearchParams = new URLSearchParams();
jest.mock("next/navigation", () => ({
  useSearchParams: () => ({
    get: (key: string) => mockSearchParams.get(key),
  }),
}));

// Mock Button component
jest.mock("@/components/general/Button", () => {
  const Component = ({
    children,
    onClick,
    variant,
    className,
  }: {
    children: React.ReactNode;
    onClick?: () => void;
    variant?: string;
    className?: string;
  }) => (
    <button
      onClick={onClick}
      data-variant={variant}
      className={className}
      data-testid={`button-${variant}`}
    >
      {children}
    </button>
  );
  Component.displayName = "Button";
  return Component;
});

// Mock Select component
jest.mock("@/components/general/Select", () => {
  const Component = ({
    label,
    instanceId,
    options,
    value,
    onChange,
    placeholder,
  }: {
    label?: string;
    instanceId?: string;
    options?: Array<{ value: string; label: string }>;
    value?: { value: string; label: string } | null;
    onChange?: (val: { value: string; label: string } | null) => void;
    placeholder?: string;
  }) => (
    <div data-testid={`select-container-${instanceId}`}>
      {label && <label>{label}</label>}
      <select
        data-testid={`select-${instanceId}`}
        value={value?.value || ""}
        onChange={(e) => {
          const selectedOption = options?.find(
            (opt) => opt.value === e.target.value
          );
          onChange?.(selectedOption || null);
        }}
      >
        <option value="">{placeholder || "Select..."}</option>
        {options?.map((opt) => (
          <option key={opt.value} value={opt.value}>
            {opt.label}
          </option>
        ))}
      </select>
    </div>
  );
  Component.displayName = "Select";
  return Component;
});

describe("TherapistFilters", () => {
  const mockOnClose = jest.fn();
  const mockOnUpdateParams = jest.fn();

  const defaultClinicOptions = [
    { value: "1", label: "Mindful Clinic" },
    { value: "2", label: "Wellness Center" },
    { value: "3", label: "Health Hub" },
  ];

  const defaultCountryOptions = [
    { value: "1", label: "Philippines" },
    { value: "2", label: "United States" },
    { value: "3", label: "Canada" },
  ];

  beforeEach(() => {
    jest.clearAllMocks();
    mockSearchParams.delete("clinic");
    mockSearchParams.delete("country");
  });

  describe("Rendering", () => {
    it("renders all filter controls", () => {
      render(
        <TherapistFilters
          onClose={mockOnClose}
          clinicOptions={defaultClinicOptions}
          countryOptions={defaultCountryOptions}
          onUpdateParams={mockOnUpdateParams}
        />
      );

      expect(screen.getByText("Clinic")).toBeInTheDocument();
      expect(screen.getByText("Country")).toBeInTheDocument();
      expect(screen.getByTestId("select-filter-clinic-select")).toBeInTheDocument();
      expect(screen.getByTestId("select-filter-country-select")).toBeInTheDocument();
      expect(screen.getByText("Clear")).toBeInTheDocument();
      expect(screen.getByText("Apply")).toBeInTheDocument();
    });

    it("renders clinic options correctly", () => {
      render(
        <TherapistFilters
          onClose={mockOnClose}
          clinicOptions={defaultClinicOptions}
          countryOptions={defaultCountryOptions}
          onUpdateParams={mockOnUpdateParams}
        />
      );

      const clinicSelect = screen.getByTestId("select-filter-clinic-select");
      const options = clinicSelect.querySelectorAll("option");

      expect(options.length).toBe(4); // placeholder + 3 clinics
      expect(options[1]).toHaveTextContent("Mindful Clinic");
      expect(options[2]).toHaveTextContent("Wellness Center");
      expect(options[3]).toHaveTextContent("Health Hub");
    });

    it("renders country options correctly", () => {
      render(
        <TherapistFilters
          onClose={mockOnClose}
          clinicOptions={defaultClinicOptions}
          countryOptions={defaultCountryOptions}
          onUpdateParams={mockOnUpdateParams}
        />
      );

      const countrySelect = screen.getByTestId("select-filter-country-select");
      const options = countrySelect.querySelectorAll("option");

      expect(options.length).toBe(4); // placeholder + 3 countries
      expect(options[1]).toHaveTextContent("Philippines");
      expect(options[2]).toHaveTextContent("United States");
      expect(options[3]).toHaveTextContent("Canada");
    });

    it("initializes with empty values when no URL params", () => {
      render(
        <TherapistFilters
          onClose={mockOnClose}
          clinicOptions={defaultClinicOptions}
          countryOptions={defaultCountryOptions}
          onUpdateParams={mockOnUpdateParams}
        />
      );

      const clinicSelect = screen.getByTestId("select-filter-clinic-select") as HTMLSelectElement;
      const countrySelect = screen.getByTestId("select-filter-country-select") as HTMLSelectElement;

      expect(clinicSelect.value).toBe("");
      expect(countrySelect.value).toBe("");
    });

    it("initializes with values from URL params", () => {
      mockSearchParams.set("clinic", "2");
      mockSearchParams.set("country", "3");

      render(
        <TherapistFilters
          onClose={mockOnClose}
          clinicOptions={defaultClinicOptions}
          countryOptions={defaultCountryOptions}
          onUpdateParams={mockOnUpdateParams}
        />
      );

      const clinicSelect = screen.getByTestId("select-filter-clinic-select") as HTMLSelectElement;
      const countrySelect = screen.getByTestId("select-filter-country-select") as HTMLSelectElement;

      expect(clinicSelect.value).toBe("2");
      expect(countrySelect.value).toBe("3");
    });

    it("handles invalid clinic param gracefully", () => {
      mockSearchParams.set("clinic", "invalid-id");

      render(
        <TherapistFilters
          onClose={mockOnClose}
          clinicOptions={defaultClinicOptions}
          countryOptions={defaultCountryOptions}
          onUpdateParams={mockOnUpdateParams}
        />
      );

      const clinicSelect = screen.getByTestId("select-filter-clinic-select") as HTMLSelectElement;
      expect(clinicSelect.value).toBe("");
    });

    it("handles invalid country param gracefully", () => {
      mockSearchParams.set("country", "999");

      render(
        <TherapistFilters
          onClose={mockOnClose}
          clinicOptions={defaultClinicOptions}
          countryOptions={defaultCountryOptions}
          onUpdateParams={mockOnUpdateParams}
        />
      );

      const countrySelect = screen.getByTestId("select-filter-country-select") as HTMLSelectElement;
      expect(countrySelect.value).toBe("");
    });

    it("supports non-latin characters in clinic and country names", () => {
      const internationalOptions = {
        clinicOptions: [
          { value: "1", label: "東京クリニック" },
          { value: "2", label: "北京医院" },
        ],
        countryOptions: [
          { value: "1", label: "日本" },
          { value: "2", label: "中国" },
          { value: "3", label: "한국" },
        ],
      };

      render(
        <TherapistFilters
          onClose={mockOnClose}
          clinicOptions={internationalOptions.clinicOptions}
          countryOptions={internationalOptions.countryOptions}
          onUpdateParams={mockOnUpdateParams}
        />
      );

      expect(screen.getByText("東京クリニック")).toBeInTheDocument();
      expect(screen.getByText("北京医院")).toBeInTheDocument();
      expect(screen.getByText("日本")).toBeInTheDocument();
      expect(screen.getByText("中国")).toBeInTheDocument();
      expect(screen.getByText("한국")).toBeInTheDocument();
    });
  });

  describe("User Interactions", () => {
    it("updates clinic when user selects an option", () => {
      render(
        <TherapistFilters
          onClose={mockOnClose}
          clinicOptions={defaultClinicOptions}
          countryOptions={defaultCountryOptions}
          onUpdateParams={mockOnUpdateParams}
        />
      );

      const clinicSelect = screen.getByTestId("select-filter-clinic-select");
      fireEvent.change(clinicSelect, { target: { value: "1" } });

      expect((clinicSelect as HTMLSelectElement).value).toBe("1");
    });

    it("updates country when user selects an option", () => {
      render(
        <TherapistFilters
          onClose={mockOnClose}
          clinicOptions={defaultClinicOptions}
          countryOptions={defaultCountryOptions}
          onUpdateParams={mockOnUpdateParams}
        />
      );

      const countrySelect = screen.getByTestId("select-filter-country-select");
      fireEvent.change(countrySelect, { target: { value: "2" } });

      expect((countrySelect as HTMLSelectElement).value).toBe("2");
    });

    it("allows changing selections multiple times", () => {
      render(
        <TherapistFilters
          onClose={mockOnClose}
          clinicOptions={defaultClinicOptions}
          countryOptions={defaultCountryOptions}
          onUpdateParams={mockOnUpdateParams}
        />
      );

      const clinicSelect = screen.getByTestId("select-filter-clinic-select");

      fireEvent.change(clinicSelect, { target: { value: "1" } });
      expect((clinicSelect as HTMLSelectElement).value).toBe("1");

      fireEvent.change(clinicSelect, { target: { value: "2" } });
      expect((clinicSelect as HTMLSelectElement).value).toBe("2");

      fireEvent.change(clinicSelect, { target: { value: "" } });
      expect((clinicSelect as HTMLSelectElement).value).toBe("");
    });

    it("clears both filters when Clear button is clicked", () => {
      mockSearchParams.set("clinic", "1");
      mockSearchParams.set("country", "2");

      render(
        <TherapistFilters
          onClose={mockOnClose}
          clinicOptions={defaultClinicOptions}
          countryOptions={defaultCountryOptions}
          onUpdateParams={mockOnUpdateParams}
        />
      );

      const clearButton = screen.getByText("Clear");
      fireEvent.click(clearButton);

      const clinicSelect = screen.getByTestId("select-filter-clinic-select") as HTMLSelectElement;
      const countrySelect = screen.getByTestId("select-filter-country-select") as HTMLSelectElement;

      expect(clinicSelect.value).toBe("");
      expect(countrySelect.value).toBe("");
      expect(mockOnUpdateParams).not.toHaveBeenCalled();
      expect(mockOnClose).not.toHaveBeenCalled();
    });

    it("calls onUpdateParams with correct values when Apply is clicked", () => {
      render(
        <TherapistFilters
          onClose={mockOnClose}
          clinicOptions={defaultClinicOptions}
          countryOptions={defaultCountryOptions}
          onUpdateParams={mockOnUpdateParams}
        />
      );

      const clinicSelect = screen.getByTestId("select-filter-clinic-select");
      const countrySelect = screen.getByTestId("select-filter-country-select");

      fireEvent.change(clinicSelect, { target: { value: "3" } });
      fireEvent.change(countrySelect, { target: { value: "1" } });

      const applyButton = screen.getByText("Apply");
      fireEvent.click(applyButton);

      expect(mockOnUpdateParams).toHaveBeenCalledWith({
        clinic: "3",
        country: "1",
        p: 1,
      });
    });

    it("calls onClose when Apply is clicked", () => {
      render(
        <TherapistFilters
          onClose={mockOnClose}
          clinicOptions={defaultClinicOptions}
          countryOptions={defaultCountryOptions}
          onUpdateParams={mockOnUpdateParams}
        />
      );

      const applyButton = screen.getByText("Apply");
      fireEvent.click(applyButton);

      expect(mockOnClose).toHaveBeenCalledTimes(1);
    });

    it("sends null values for unselected filters when Apply is clicked", () => {
      render(
        <TherapistFilters
          onClose={mockOnClose}
          clinicOptions={defaultClinicOptions}
          countryOptions={defaultCountryOptions}
          onUpdateParams={mockOnUpdateParams}
        />
      );

      const applyButton = screen.getByText("Apply");
      fireEvent.click(applyButton);

      expect(mockOnUpdateParams).toHaveBeenCalledWith({
        clinic: null,
        country: null,
        p: 1,
      });
    });

    it("resets page to 1 when filters are applied", () => {
      mockSearchParams.set("p", "8");

      render(
        <TherapistFilters
          onClose={mockOnClose}
          clinicOptions={defaultClinicOptions}
          countryOptions={defaultCountryOptions}
          onUpdateParams={mockOnUpdateParams}
        />
      );

      const clinicSelect = screen.getByTestId("select-filter-clinic-select");
      fireEvent.change(clinicSelect, { target: { value: "2" } });

      const applyButton = screen.getByText("Apply");
      fireEvent.click(applyButton);

      expect(mockOnUpdateParams).toHaveBeenCalledWith({
        clinic: "2",
        country: null,
        p: 1,
      });
    });

    it("handles applying only clinic filter", () => {
      render(
        <TherapistFilters
          onClose={mockOnClose}
          clinicOptions={defaultClinicOptions}
          countryOptions={defaultCountryOptions}
          onUpdateParams={mockOnUpdateParams}
        />
      );

      const clinicSelect = screen.getByTestId("select-filter-clinic-select");
      fireEvent.change(clinicSelect, { target: { value: "1" } });

      const applyButton = screen.getByText("Apply");
      fireEvent.click(applyButton);

      expect(mockOnUpdateParams).toHaveBeenCalledWith({
        clinic: "1",
        country: null,
        p: 1,
      });
    });

    it("handles applying only country filter", () => {
      render(
        <TherapistFilters
          onClose={mockOnClose}
          clinicOptions={defaultClinicOptions}
          countryOptions={defaultCountryOptions}
          onUpdateParams={mockOnUpdateParams}
        />
      );

      const countrySelect = screen.getByTestId("select-filter-country-select");
      fireEvent.change(countrySelect, { target: { value: "3" } });

      const applyButton = screen.getByText("Apply");
      fireEvent.click(applyButton);

      expect(mockOnUpdateParams).toHaveBeenCalledWith({
        clinic: null,
        country: "3",
        p: 1,
      });
    });
  });

  describe("Button Variants", () => {
    it("renders Clear button with outline variant", () => {
      render(
        <TherapistFilters
          onClose={mockOnClose}
          clinicOptions={defaultClinicOptions}
          countryOptions={defaultCountryOptions}
          onUpdateParams={mockOnUpdateParams}
        />
      );

      const clearButton = screen.getByTestId("button-outline");
      expect(clearButton).toHaveTextContent("Clear");
      expect(clearButton).toHaveAttribute("data-variant", "outline");
    });

    it("renders Apply button with filled variant", () => {
      render(
        <TherapistFilters
          onClose={mockOnClose}
          clinicOptions={defaultClinicOptions}
          countryOptions={defaultCountryOptions}
          onUpdateParams={mockOnUpdateParams}
        />
      );

      const applyButton = screen.getByTestId("button-filled");
      expect(applyButton).toHaveTextContent("Apply");
      expect(applyButton).toHaveAttribute("data-variant", "filled");
    });
  });

  describe("Edge Cases", () => {
    it("handles empty clinic options array", () => {
      render(
        <TherapistFilters
          onClose={mockOnClose}
          clinicOptions={[]}
          countryOptions={defaultCountryOptions}
          onUpdateParams={mockOnUpdateParams}
        />
      );

      const clinicSelect = screen.getByTestId("select-filter-clinic-select");
      const options = clinicSelect.querySelectorAll("option");

      expect(options.length).toBe(1); // Only placeholder
    });

    it("handles empty country options array", () => {
      render(
        <TherapistFilters
          onClose={mockOnClose}
          clinicOptions={defaultClinicOptions}
          countryOptions={[]}
          onUpdateParams={mockOnUpdateParams}
        />
      );

      const countrySelect = screen.getByTestId("select-filter-country-select");
      const options = countrySelect.querySelectorAll("option");

      expect(options.length).toBe(1); // Only placeholder
    });

    it("handles both empty arrays", () => {
      render(
        <TherapistFilters
          onClose={mockOnClose}
          clinicOptions={[]}
          countryOptions={[]}
          onUpdateParams={mockOnUpdateParams}
        />
      );

      expect(screen.getByTestId("select-filter-clinic-select")).toBeInTheDocument();
      expect(screen.getByTestId("select-filter-country-select")).toBeInTheDocument();
    });

    it("handles rapid filter changes before applying", () => {
      render(
        <TherapistFilters
          onClose={mockOnClose}
          clinicOptions={defaultClinicOptions}
          countryOptions={defaultCountryOptions}
          onUpdateParams={mockOnUpdateParams}
        />
      );

      const clinicSelect = screen.getByTestId("select-filter-clinic-select");
      const countrySelect = screen.getByTestId("select-filter-country-select");

      fireEvent.change(clinicSelect, { target: { value: "1" } });
      fireEvent.change(clinicSelect, { target: { value: "2" } });
      fireEvent.change(countrySelect, { target: { value: "1" } });
      fireEvent.change(countrySelect, { target: { value: "3" } });

      const applyButton = screen.getByText("Apply");
      fireEvent.click(applyButton);

      // Should use the last selected values
      expect(mockOnUpdateParams).toHaveBeenCalledWith({
        clinic: "2",
        country: "3",
        p: 1,
      });
    });

    it("handles clearing after selecting and before applying", () => {
      render(
        <TherapistFilters
          onClose={mockOnClose}
          clinicOptions={defaultClinicOptions}
          countryOptions={defaultCountryOptions}
          onUpdateParams={mockOnUpdateParams}
        />
      );

      const clinicSelect = screen.getByTestId("select-filter-clinic-select");
      const countrySelect = screen.getByTestId("select-filter-country-select");

      // Select values
      fireEvent.change(clinicSelect, { target: { value: "1" } });
      fireEvent.change(countrySelect, { target: { value: "2" } });

      // Clear
      const clearButton = screen.getByText("Clear");
      fireEvent.click(clearButton);

      // Apply
      const applyButton = screen.getByText("Apply");
      fireEvent.click(applyButton);

      expect(mockOnUpdateParams).toHaveBeenCalledWith({
        clinic: null,
        country: null,
        p: 1,
      });
    });

    it("maintains selections after clearing without applying", () => {
      mockSearchParams.set("clinic", "1");
      mockSearchParams.set("country", "2");

      render(
        <TherapistFilters
          onClose={mockOnClose}
          clinicOptions={defaultClinicOptions}
          countryOptions={defaultCountryOptions}
          onUpdateParams={mockOnUpdateParams}
        />
      );

      // Clear the form
      const clearButton = screen.getByText("Clear");
      fireEvent.click(clearButton);

      // Selections should be cleared in UI
      const clinicSelect = screen.getByTestId("select-filter-clinic-select") as HTMLSelectElement;
      const countrySelect = screen.getByTestId("select-filter-country-select") as HTMLSelectElement;

      expect(clinicSelect.value).toBe("");
      expect(countrySelect.value).toBe("");
    });

    it("handles applying filters without making any changes", () => {
      mockSearchParams.set("clinic", "2");
      mockSearchParams.set("country", "1");

      render(
        <TherapistFilters
          onClose={mockOnClose}
          clinicOptions={defaultClinicOptions}
          countryOptions={defaultCountryOptions}
          onUpdateParams={mockOnUpdateParams}
        />
      );

      const applyButton = screen.getByText("Apply");
      fireEvent.click(applyButton);

      expect(mockOnUpdateParams).toHaveBeenCalledWith({
        clinic: "2",
        country: "1",
        p: 1,
      });
      expect(mockOnClose).toHaveBeenCalled();
    });

    it("handles single long clinic name", () => {
      const longClinicOptions = [
        {
          value: "1",
          label: "The Very Long Name of an International Medical Wellness and Mental Health Treatment Center",
        },
      ];

      render(
        <TherapistFilters
          onClose={mockOnClose}
          clinicOptions={longClinicOptions}
          countryOptions={defaultCountryOptions}
          onUpdateParams={mockOnUpdateParams}
        />
      );

      expect(
        screen.getByText(
          "The Very Long Name of an International Medical Wellness and Mental Health Treatment Center"
        )
      ).toBeInTheDocument();
    });

    it("handles selecting and deselecting the same filter", () => {
      render(
        <TherapistFilters
          onClose={mockOnClose}
          clinicOptions={defaultClinicOptions}
          countryOptions={defaultCountryOptions}
          onUpdateParams={mockOnUpdateParams}
        />
      );

      const clinicSelect = screen.getByTestId("select-filter-clinic-select");

      fireEvent.change(clinicSelect, { target: { value: "1" } });
      expect((clinicSelect as HTMLSelectElement).value).toBe("1");

      fireEvent.change(clinicSelect, { target: { value: "" } });
      expect((clinicSelect as HTMLSelectElement).value).toBe("");

      const applyButton = screen.getByText("Apply");
      fireEvent.click(applyButton);

      expect(mockOnUpdateParams).toHaveBeenCalledWith({
        clinic: null,
        country: null,
        p: 1,
      });
    });
  });
});
